...
//printf(":%s:\n", line);
  ...
  for (i = 0, tarelf = curelf; i < num_elves / 2; i++) {
    tarelf = list_entry(tarelf->circle.next, elf_t, circle);
  }
  while (curelf != tarelf) {
    curelf->presents += tarelf->presents;
//printf("[%d] Elf %d takes Elf %d's %d presents and has %d presents now\n", num_elves, curelf->idx,
//tarelf->idx, tarelf->presents, curelf->presents);
    tmpelf = tarelf;
    tarelf = list_entry(tarelf->circle.next, elf_t, circle);
