...
printf("5REP %d:%c\n", j, newkey->md5str[j]);
    ...
    if (j < MD5_DIGEST_LENGTH_A - 4) {
      // update the potential keys list
...
printf("KEY %d\n", num_keys);
      ...
      // finish processing 5reps
      for (k = 0; k < 16; k++)
        window_checked_for[k] = false;

      while (j < MD5_DIGEST_LENGTH_A - 4) {
...
printf("\n");
      ...
      list_for_each_safe(potkeyptr, tmpkeyptr, &potential_keys) {
        potkey = list_entry(potkeyptr, santa_key_t, list);
printf("CONSIDER [%d] [%s] [%d] [%d] ", potkey->idx, potkey->md5str, potkey->triple, potkey->rep5);
        if (potkey->rep5 == -1)
          break;
        list_del(potkeyptr);
        list_add(potkeyptr, &keys);
